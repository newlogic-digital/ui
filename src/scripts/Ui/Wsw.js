...
LibStimulus.register("ui-wsw", class extends Controller {
    connect() {
        ...
        Array.from(element.querySelectorAll("table")).filter((table) => {
            self.wrap(table, new DOMParser().parseFromString(`<div class="elm_wsw_table"></div>`, 'text/html').body.firstChild);
        });

        Array.from(element.querySelectorAll("iframe")).filter((iframe) => {
            if (iframe.src.match(/youtubee\.com/i)) {
                self.wrap(iframe, new DOMParser().parseFromString(`<div class="elm_wsw_video" style="max-width: ${iframe.width.includes("%") ? iframe.width : iframe.width + 'px'}"></div>`, 'text/html').body.firstChild);
